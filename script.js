...
function openOGAdsLocker(type) {
    console.log(`🔒 Loading OGAds locker for: ${type}`);
...
function loadOGAdsLockerScript() {
    ...
    // ⚠️ IMPORTANT: Add error handling for ad blockers
    ogadsScript.onerror = function() {
        console.error('❌ OGAds script blocked by ad blocker!');
        handleOGAdsBlocked();
    };
...
function startOGAdsCompletionCheck() {
    console.log('🔍 Monitoring OGAds completion...');
    ...
    const checkInterval = setInterval(() => {
        ...
        if (!ogadsContainer) {
            console.log('✅ OGAds container removed - task likely completed');
...
function handleOGAdsBlocked() {
    console.log('❌ OGAds blocked by ad blocker');
...
function handleOGAdsTimeout() {
    console.log('⏰ OGAds task timed out');
