...
function startMiningWithTimer() {
    ...
    miningTimer = setInterval(() => {
        ...
        // ✅ CRITICAL: When timer reaches 0, OPEN OGADS
        if (miningTimeLeft <= 0) {
...
// ⚡ OPTIMIZED: Changed from 100ms to 500ms interval
function startMining() {
    ...
    miningInterval = setInterval(() => {
        if (minedTokens < DAILY_LIMIT) {
            const tokensPerSecond = miningSpeed / 10000; // Adjusted for 500ms
...
function showOGAdsVerification(type) {
            ...
            <div style="background: linear-gradient(135deg, #1a1a2e, #16213e);
                ...
                <p style="margin-bottom: 25px; color: #aaa;">
                    ...
                    <span style="color: #e74c3c; display: block; margin-top: 10px;">
                        ⚠️ Ad blockers will prevent continuation!
                    </span>
                </p>
                <div style="background: rgba(42, 75, 141, 0.2); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
                    ...
                    <div id="ogads-monitoring-status" style="color: #aaa; font-size: 0.9rem;">
                        Waiting for OGAds verification...
                    </div>
                </div>
    ...
    // Add force complete function
    window.forceCompleteOGAds = function() {
        if (confirm("Skip task? (For testing only - no revenue)")) {
            completeOGAdsTask(window.currentOGAdsType);
        }
    };
}
...
function startOGAdsMonitoring(type) {
    ...
    const monitoringInterval = setInterval(() => {
        ...
        if (statusEl) {
            statusEl.textContent = `Verifying... (${checkCount}/300s)`;
        }
        
        // Check if OGAds redirected to adblock page (ad blocker detected)
        ...
        // Check if OGAds container is gone (user completed task)
        ...
        // Timeout after 5 minutes - USER FAILED
        if (checkCount >= maxChecks) {
            clearInterval(monitoringInterval);
            handleTaskFailure(type);
        }
    }, 1000);
}
...
function handleOGAdsBlocked(type) {
    console.log('❌ Ad blocker detected by OGAds!');
...
function handleTaskFailure(type) {
    console.log(`❌ Task failed for: ${type}`);
