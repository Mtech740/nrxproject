...
let minedTokens = 0;
let miningSpeed = 20; // Base speed - much slower now
let isMining = false;
let miningInterval;
...
function openOGADSModal(action) {
    ...
    // The iframe source is already set in HTML, no need to set it here
}
    ...
    if (!isMining) {
        ...
        miningInterval = setInterval(function() {
            if (minedTokens < 20) {
                ...
                minedTokens += (miningSpeed / 50000); // Reduced from 10000 to 50000
                document.getElementById('mined-tokens').textContent = minedTokens.toFixed(4) + ' NRX';
            } else {
