...
let pendingWithdrawal = null; // Store withdrawal details
...
closeBtn.onclick = function() {
    ...
    // Reset pending withdrawal if user closes OGADS modal
...
taskCompletedBtn.onclick = function() {
    ...
    if (currentAction === 'boost') {
        ...
        // Process withdrawal only after OGADS task completion
...
confirmWithdrawalBtn.onclick = function() {
    ...
    // Store withdrawal details for processing after OGADS task
    pendingWithdrawal = {
...
document.getElementById('start-mining-btn').addEventListener('click', function() {
    if (!isMining) {
        ...
        miningInterval = setInterval(function() {
            if (minedTokens < 20) {
                // SLOWED DOWN: Now takes about 8-10 hours to reach daily limit
